...
// Grab NODE_ENV and KKT_* environment variables and prepare them to be
// injected into the application via DefinePlugin in Webpack configuration.
const KKT = /^KKT_/i;

function getClientEnvironment(target) {
  const raw = Object.keys(process.env)
    .filter(key => KKT.test(key))
    .reduce(
      (env, key) => {
      ...
      },
      {
        ...
        // It's useful if you're running KKT in a non-localhost container. Ends in a /
